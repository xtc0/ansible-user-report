...
- Filter out system users to only include real accounts
- Generate a well-formatted HTML report
- Email reports via Gmail

It is useful for network engineers who want a **repeatable and automated way** to collect device status and configurations.

> **Note:** Requires Ansible with the `community.general` collection installed for mailing.

---

## Requirements

- Ansible installed beforehand
...
```bash
git clone https://github.com/username/ansible-user-reports.git
cd ansible-user-reports
```
---
...
```bash
# To run the playbook
ansible-playbook local_users.yml --ask-vault-pass 
```
